...
// Generate SIMD wrapper files for BLAKE3.  Each SIMD backend must be compiled
// as its own translation unit so that #pragma GCC target applies cleanly to all
// the functions defined in that file.  The wrapper just sets the target pragma
// then #includes the source file from doltlite-src/ext/blake3/.
const simdWrappers = [
  // name in amalgamation/        source file          GCC target string     arch guard
  ["blake3_sse2_impl.c",   "blake3_sse2.c",   "sse2",             "defined(__x86_64__)||defined(_M_X64)||defined(__i386__)"],
  ["blake3_sse41_impl.c",  "blake3_sse41.c",  "sse4.1",           "defined(__x86_64__)||defined(_M_X64)||defined(__i386__)"],
  ["blake3_avx2_impl.c",   "blake3_avx2.c",   "avx2",             "defined(__x86_64__)||defined(_M_X64)"],
  ["blake3_avx512_impl.c", "blake3_avx512.c", "avx512f,avx512vl", "defined(__x86_64__)||defined(_M_X64)"],
  // NEON: no pragma needed (always available on aarch64/armv7+neon)
  ["blake3_neon_impl.c",   "blake3_neon.c",   null,               "defined(__aarch64__)||defined(__arm__)"],
]

for (const [wrapName, srcName, gcctarget, archGuard] of simdWrappers) {
  ...
  if (gcctarget) {
    // Clang requires #pragma clang attribute; GCC uses push_options/target.
    // MSVC exposes all intrinsics by default — no pragma needed.
    lines.push(
      "#if defined(__clang__)",
      `#pragma clang attribute push (__attribute__((target("${gcctarget}"))), apply_to=function)`,
      "#elif defined(__GNUC__)",
      "#pragma GCC push_options",
      `#pragma GCC target("${gcctarget}")`,
      "#endif"
    )
  }
  lines.push(`#include "${relSrc}"`)
  if (gcctarget) {
    lines.push(
      "#if defined(__clang__)",
